	...
	void Update ()    
	{
		...
		if (Input.GetMouseButton (2) || Input.GetKey(KeyCode.JoystickButton9) ) 
		{
			...
			{                
				...
				FreezedBlock.transform.position = new Vector3(FreezedBlock.transform.position.x,
					FreezedBlock.transform.position.y +0.1f,FreezedBlock.transform.position.z);
		...
		{        
			if(ObjectFreezed){
				...
				FreezedBlock.rb.mass = 1000000000f ;
				//FreezedBlock.gameObject.AddComponent<Rigidbody> ().mass = 1000000000f ;
				//= 1000000000f ;
